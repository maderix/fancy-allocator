// Benchmark for Fancy allocator only
...
int main() {
    ...
    std::cout << "=== Fancy Allocator Benchmark ===" << std::endl;
    std::cout << "Threads: " << threads << ", Ops/Thread: " << opsPerThread << std::endl;

    // Create allocator with reclamation ON
    // 64MB per thread - best perf/memory trade-off (some large allocs will fail)
